...
#11."AtBat"       "Hits"        "Walks"       "CAtBat"      "CRuns"       "CRBI"        "CWalks"      "LeagueN"     "DivisionW"   "PutOuts"     "Assists" 
# Question: Which subset of variables achieves the best Cp score?
#10. "AtBat"       "Hits"        "Walks"       "CAtBat"     "CRuns"       "CRBI"        "CWalks"      "DivisionW"   "PutOuts"     "Assists"
# Question: Which subset of variables achieves the best BIC score?
#6."AtBat"       "Hits"        "Walks"       "CRBI"        "DivisionW"   "PutOuts"
# _____________________________________________________________________________
...
# This one contain the same variables as the model selected by BIC.
# _____________________________________________________________________________
...
# This one contain the same variables as the model selected by Cp score.
# Question: Does this make sense based on what AIC is "trying" to do?
#Yes, since the AIC assess the likelihood and number of the variables, and this in fact is the bias and the variance of the model, thus the validation method is same as the AIC selection.
...
# The mse from cross-validation only gives us the number of variables since the every k-subset we may have different subset of variables. Thus, we need fit the model on the whole data set to get the best subset for the model.
# Question: Which would you trust more, using a single train/test split or cross-validation?
# Cross-validation, since it fits the model multiple times.
...
# The estimated coefficients get smaller as the lambda increases.
...
#since the coefficients is very close to 0 when the lambda is large, thus it will be very close to the intercept-only model.
...
#The mse for the bestlam is less than that of model with lambda=4.
# Question: Are any of the coefs exactly zero?
# No, they are not exactly zero.
...
# As the lambda increases, the coef of the variables decreases and finally equal to 0.
...
#The mse of the lasso is larger than the Mse of ridge when using the bestlam for each models.
...
#        Hits        Walks        CRuns         CRBI      LeagueN   DivisionW      PutOuts
...
#both included: Hits Walks DivisionW PutOuts
#only in lasso: Cruns CRBI  LeagueN
#only in bestset: caTbAT CHits CHmRun
...
# No, some variables has small p-values but they are not included in Lasso.
...
load(file="/Users/jiangshanzhang/Downloads/lab5-train.rda")
...
f_predict = function(X.test) { 
  grid=10^seq(10,-2,length=100)
  lasso.mod=glmnet(as.matrix(X),as.matrix(y),alpha=0.95,lambda=grid)
  cv.out=cv.glmnet(as.matrix(X),as.matrix(y),alpha=0.95)
  bestlam=cv.out$lambda.min
  bestlam
...
load(file="/Users/jiangshanzhang/Downloads/lab-5-test.rda")
X.test = D.test[,-1]
y.test = D.test[,1]
...
msea=rep(0,11)
asel=rep(0,11)
for(a in seq(0.925,0.975,0.005)){
  lasso.mod=glmnet(as.matrix(X),as.matrix(y),alpha=a,lambda=grid)
  cv.out=cv.glmnet(as.matrix(X),as.matrix(y),alpha=a)
  bestlam=cv.out$lambda.min
  bestlam
  asel[200*a-184]=a
  # Assess MSE on the test set when using bestlam
  lasso.pred=predict(lasso.mod,s=bestlam,newx=as.matrix(X))
  msea[200*a-184]=mean((lasso.pred-y)^2)
